...
def make_predictions(model, band_file, bands, device, nclass=3):
    ...
    for h in range(n_h):
        for w in range(n_w):
            ...
            c_image[(h_max - _size):h_max, (w_max - _size):w_max, :] = np.max(
                np.stack([preds[idx, :, :, :], c_image[(h_max - _size):h_max, (w_max - _size):w_max, :]], axis=-1),
                axis=-1)
...
def save_geotiff(reference_tif, preds, output_path):
    """
    Guarda el mapa de clases 'preds' como un GeoTIFF georreferenciado,
    copiando la geometría y transformada del TIFF original.
    """
    import rasterio
    import numpy as np

    # Abro la imagen original para copiar metadata
    with rasterio.open(reference_tif) as src:
        meta = src.meta.copy()

    # Ajuste de metadatos para imagen de 1 banda (clases)
    meta.update({
        "count": 1,
        "dtype": "uint8"
    })
...
def main():
    ...
    # Dispositivo (GPU si hay, si no CPU)
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    print("Usando dispositivo:", device)

    # Cargo el modelo directamente completo desde el .pth
    net = torch.load(
        "./models/unet_4bands.pth",
        map_location=device,
        weights_only=False  # Paraque funcione PyTorch 2.8
    )

    net.to(device)
    net.eval()

    # Diccionario nombre de banda → índice (1–12) en el GeoTIFF
    bands_dict = dict(zip(
        ['B1', 'B2', 'B3', 'B4', 'B5', 'B6',
    ...
    # Selecciono de las 4 bandas que usa el modelo
    ...
    # Guardo en GeoTiff TIFF en Google Drive
    tif_output = "/content/drive/MyDrive/TFM/Andorra_SnowCoverage_20230107.tif"
    save_geotiff(in_file, preds, tif_output)
